/**
 * Conference Intelligence Hub — Task Notification Script
 * 
 * HOW TO SET UP:
 ...
 * 9. Paste the URL in Dashboard Settings → Email Notifications
 */
...
function doPost(e) {
  try {
    ...
    var subject = priorityEmoji + ' New Task: ' + task + (conference ? ' — ' + conference : '');
    
    var body = 'Hi ' + assignee + ',\n\n' +
      'You have been assigned a new task in the Conference Intelligence Hub.\n\n' +
      '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n' +
      '📋 Task: ' + task + '\n' +
      (conference ? '🏛️ Conference: ' + conference + '\n' : '') +
    ...
    var htmlBody = '<div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:600px;margin:0 auto;">' +
      ...
      '<div style="background:#ffffff;border:1px solid #e8ecf2;border-top:none;padding:24px;border-radius:0 0 12px 12px;">' +
        ...
        '<p style="margin:0 0 20px;color:#6b7b94;">You\'ve been assigned a task in the Conference Intelligence Hub.</p>' +
        '<div style="background:#f7f9fc;border-radius:10px;padding:20px;margin-bottom:20px;">' +
          ...
          '<table style="width:100%;font-size:14px;color:#6b7b94;">' +
            (conference ? '<tr><td style="padding:4px 0;">🏛️ Conference</td><td style="padding:4px 0;font-weight:500;color:#1a2233;">' + conference + '</td></tr>' : '') +
            '<tr><td style="padding:4px 0;">' + priorityEmoji + ' Priority</td><td style="padding:4px 0;font-weight:500;color:#1a2233;">' + priority.charAt(0).toUpperCase() + priority.slice(1) + '</td></tr>' +
...
function doGet(e) {
  return ContentService.createTextOutput(JSON.stringify({ status: 'ok', message: 'Conference Intelligence Hub Email Notifier is running.' }))
    .setMimeType(ContentService.MimeType.JSON);
}
